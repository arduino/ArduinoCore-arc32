...
extern int service_mgr_port_id;
...
int cfw_open_service(cfw_handle_t handle, int service_id, void * priv) {
    ...
    CFW_MESSAGE_DST(&msg->header) = service_mgr_port_id;
...
int cfw_register_svc_available(cfw_handle_t handle, void *priv)
{
    struct cfw_message * msg = cfw_alloc(sizeof(*msg), NULL);
    CFW_MESSAGE_ID(msg) = MSG_ID_CFW_REGISTER_SVC_AVAIL;
    CFW_MESSAGE_LEN(msg) = sizeof(*msg);
    CFW_MESSAGE_SRC(msg) = ((_cfw_handle_t*)handle)->client_port_id;
    CFW_MESSAGE_DST(msg) = service_mgr_port_id;
...
struct cfw_message * cfw_alloc_message_for_service(svc_client_handle_t * h, int msg_id, int msg_size,
        void * priv) {
    struct cfw_message * msg = (struct cfw_message*) cfw_alloc(msg_size, NULL);
...
{
	return (struct cfw_message *)message_alloc(size, err);
}
