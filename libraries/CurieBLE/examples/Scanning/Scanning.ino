...
/*
  ...
  are present because this sketch allows to use your Arduino/Genuino 101 as a Central device that scans
...
typedef struct {
  ...
  char localname[22];    // Device's name
} ble_device_info_t;
...
void loop()
{
  ...
  for (int i = 0; i < list_index; i++)
  {
    ...
    Serial.print(" | ");
    Serial.println(device_list[i].localname);
  }
  if (list_index == 0)
  {
...
bool adv_list_update(ble_device_info_t &device)
{
  for (int i = 0; i < list_index; i++)
  {
    ...
    {
      // Found and update the item
      memcpy(device_list[i].localname, device.localname, sizeof(device.localname));
      return true;
    }
  }
  return false;
