...
{
	// add two extra buffer to balance the padding zero transmit
	uint32_t offset = 8;
	soc_i2s_stream(buf_TX, len+offset,0); 
...
{
	uint32_t offset = 0;
	if(rxcfg.master == 0 && (rxcfg.mode == I2S_MODE_RJ || rxcfg.mode == I2S_MODE_LJ))
		offset = 8;
	
	soc_i2s_listen(buf_RX, len-offset ,0);
